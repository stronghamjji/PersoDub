...
import { existsSync, mkdirSync, cpSync, writeFileSync, symlinkSync, rmSync, readFileSync } from "node:fs";
...
export function writeMacEnv({ kitDir }) {
  ...
  return [
    ...
    `SEP_PYTHON=${k("engines_venv", "bin", "python")}`,
    `STT_PYTHON=${k("engines_venv", "bin", "python")}`,
    `DIAR_PYTHON=${k("engines_venv", "bin", "python")}`,
    `QWEN_SCORER_PYTHON=${k("engines_venv", "bin", "python")}`,
    ...
    `NONVERBAL_WHISPER_PYTHON=${k("engines_venv", "bin", "python")}`,
...
export function buildSteps(ctx) {
  ...
  const py = k("python", "bin", "python3.11");
  ...
  const venvStep = (id, title, venvName, pipInstalls) => ({
    ...
    run: async (report) => {
      ...
      await ctx.run([pip, "install", "--upgrade", "pip"], { onLine: (l) => report(null, l.slice(0, 120)) });
  ...
  return [
    {
      ...
      run: async (report) => {
        ...
        for (const f of ["requirements_engines_mac.txt", "requirements_qwen_mac.txt"]) {
    ...
    venvStep("venv-engines", "Installing AI engines (~3 GB)", "engines_venv", [
      ["-r", k("requirements_engines_mac.txt")],
      ["static-ffmpeg"],
    ]),
    {
      id: "ffmpeg",
      title: "Setting up ffmpeg",
      isDone: () => existsSync(k("bin", "ffmpeg")) && existsSync(k("bin", "ffprobe")),
      run: async (report) => {
        report(null, "Fetching ffmpeg binaries");
        const lines = [];
        await ctx.run(
          [
            k("engines_venv", "bin", "python"),
            "-c",
    ...
    },
    {
      ...
      isDone: () => existsSync(k(...GEMMA_MANIFEST)) && existsSync(k("ollama", "ollama")),
      run: async (report) => {
        const bin = k("ollama", "ollama");
        if (!existsSync(bin)) {
          report(null, "Downloading Ollama runtime (~120 MB)");
          mkdirSync(k("downloads"), { recursive: true });
          const tgz = k("downloads", "ollama.tgz");
          await ctx.download(OLLAMA_TGZ_URL, tgz, {
          ...
          await ctx.extract(tgz, k("ollama"));
        }
    ...
    },
    {
      ...
      run: async (report) => {
        ...
        await ctx.run([k("engines_venv", "bin", "python"), "-c", "import whisper; whisper.load_model('base')"]);
      },
    },
    {
      id: "mac-env",
