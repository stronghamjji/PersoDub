...
// ---- the one-time notice: existing users installed on "no telemetry" ----

import { shouldShowNotice, noticeSeen, markNoticeSeen } from "./analytics.js";

function tempNoticeFile() {
  return join(mkdtempSync(join(tmpdir(), "persodub-notice-")), "notices.json");
}

test("a machine that is about to report is told so, once", () => {
  assert.equal(shouldShowNotice({ mode: "on", seen: false }), true);
});

test("a machine that has already been told is not told again", () => {
  assert.equal(shouldShowNotice({ mode: "on", seen: true }), false);
});

test("a machine with reporting already off is never told it started", () => {
  // The notice says counts are being sent. On a machine that turned them off
  // -- or a from-source run, which never reports -- that sentence is false.
  assert.equal(shouldShowNotice({ mode: "off", seen: false }), false);
});

test("a debug run is not told counts are being sent, because they are not", () => {
  assert.equal(shouldShowNotice({ mode: "debug", seen: false }), false);
});

test("an install that has never been told has no mark", () => {
  assert.equal(noticeSeen(tempNoticeFile()), false);
});

test("the mark survives a restart, so the notice does not come back", () => {
  const file = tempNoticeFile();
  markNoticeSeen(file);
  assert.equal(noticeSeen(file), true);
});

test("an unreadable mark counts as not shown -- better to repeat than to hide", () => {
  const file = tempNoticeFile();
  writeFileSync(file, "{ this is not json");
  assert.equal(noticeSeen(file), false);
});

test("marking cannot throw on a path that cannot be written", () => {
  // A read-only disk costs the mark, not the launch.
  assert.doesNotThrow(() => markNoticeSeen("/dev/null/nope/notices.json"));
});
