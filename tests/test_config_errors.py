...
reported -- at ERROR, without stopping the launch. The reload below is what
proves "recorded, not raised":
...
def test_startup_logs_the_bad_setting_at_error(monkeypatch, caplog):
    """The reason is reported, once, and the app carries on with the default.

    A refusal to start would reach a desktop user as a window that never
    opens: the shell hands the server's output to a log file, not the screen.
    So the loud thing is the log line, not the exit.
    ...
    into an ExceptionGroup, and this test is about what OUR code does."""
    ...
    with caplog.at_level(logging.ERROR, logger="persodub.main"):
        asyncio.run(start())

    errors = [r.getMessage() for r in caplog.records if r.levelno >= logging.ERROR]
    assert any("QWEN_N_TAKES" in message for message in errors), errors


def test_the_app_still_starts_on_a_bad_setting(monkeypatch):
    monkeypatch.setattr(config_module, "CONFIG_ERRORS",
                        ["QWEN_N_TAKES: got 'abc', expected an integer (default 4)"])
